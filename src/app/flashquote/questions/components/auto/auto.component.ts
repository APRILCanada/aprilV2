...
export class AutoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.filteredMakeOptions$ = this.group$.pipe(
      ...
      map((value: any) => value ? this._filterMakes(value) : this.makes))

    this.filteredModelOptions$ = this.group$.pipe(
      pluck('Vehicle-Model', 'value'),
      map((value: any) => value ? this._filterModel(value) : this.models));
  ...
  private _filterMakes(value: CarMake | string): CarMake[] {
    const val = value instanceof CarMake ? value.make : value;
    const filterValue = val.toString().toLowerCase();

    if (this.selectedYear === "")
      return [];
    const makes = this.makes.filter(opt => {
      const label = opt.make.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "");
      return label.startsWith(filterValue);
    });
    return makes;
  }

  private _filterModel(val: string): CarModel[] {
